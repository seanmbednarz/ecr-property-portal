...
CREATE POLICY "select_property_photos" ON property_photos FOR SELECT TO authenticated USING (true);
CREATE POLICY "insert_property_photos" ON property_photos FOR INSERT TO authenticated WITH CHECK (auth.uid() = created_by);
CREATE POLICY "update_property_photos" ON property_photos FOR UPDATE TO authenticated USING (auth.uid() = created_by) WITH CHECK (auth.uid() = created_by);
CREATE POLICY "delete_property_photos" ON property_photos FOR DELETE TO authenticated USING (auth.uid() = created_by);
