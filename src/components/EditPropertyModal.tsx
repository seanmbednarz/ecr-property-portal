...
export default function EditPropertyModal({ property, onClose, onSaved, onDeleted, clients = [] }: EditPropertyModalProps) {
  ...
  function addSuite() {
    setSuites(prev => [...prev, { id: null, suite_name: '', listing_type: 'lease', sf: '', base_rent: '', op_exp: '', sale_price: '', available: 'Available Now', tour_url: '', client_ids: [] }]);
  }
  ...
  async function handleSave() {
    ...
    try {
      ...
      for (let i = 0; i < suites.length; i++) {
        const s = suites[i];
        const isSale = s.listing_type === 'sale';
        const row = {
          ...
          op_exp: !isSale && s.op_exp ? parseFloat(s.op_exp) : null,
          sale_price: isSale && s.sale_price ? parseFloat(s.sale_price) : null,
          available: s.available || 'Available Now',
